...
export function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex flex-col h-screen w-60 bg-[#13131A] border-r border-[#2A2A3A] px-3 py-4 shrink-0">
      ...
      {/* Mobile bottom nav */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#13131A] border-t border-[#2A2A3A] flex items-center px-2 safe-bottom">
        {nav.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href}>
            <a
              className={cn(
                "flex flex-col items-center gap-1 flex-1 py-2.5 text-xs transition-colors",
                location === href ? "text-[#6366F1]" : "text-[#6B7280]"
              )}
            >
              <Icon size={20} />
              {label}
        ...
        <button
          onClick={logout}
          className="flex flex-col items-center gap-1 flex-1 py-2.5 text-xs text-[#6B7280]"
        >
          <LogOut size={20} />
          Выйти
        </button>
